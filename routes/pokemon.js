...
pokemon.post("/", async (req,res,next) =>{
   ...
   if(pok_name && pok_height && pok_weight && pok_base_experience){
        ...
        if(rows.affectedRows == 1){
            return res.status(201).json({code: 201, menssage: "pokemon insertado correctamente"});
        }
...
pokemon.put("/:id([0-9]{1,3})",async(req, res, next) =>{
    ...
    if(pok_name && pok_height && pok_weight && pok_base_experience){
         ...
         if(rows.affectedRows == 1){
             return res.status(200).json({code: 200, menssage: "pokemon actualizado correctamente"});
         }
...
pokemon.get('/', async (req,res,next) =>{
    ...
    return res.status(200).json({code: 200, menssage: pkmn});
...
pokemon.get('/:id([0-9]{1,3})', async (req, res, next) => {
    ...
    return res.status(404).json({code: 404, menssage: "pokemon no encontrado"});
